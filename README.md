...
Structs/Classes: Made classes for boss, and player character each classes have HP, attack damage, and name. You can initiate those variables under int main() by using setMaxHealth(), setAttDamage and setName(), but it also has default varariables. You can see those values by doing getMaxHealth(), getAttDamage(), and getName().


Create puzzles

Algebra: Different varaibles are constanltly referenced throughout the code to do different things. 



Loop: In puzzles 2 and 3, while loops are used to reiterate certain parts of code until the correct input. Also in puzzle 3, a for loop is placed to make every input from the user lowercase. Loops are also used for gameplay mechanics.
...
Puzzles - 5 puzzles for locks. Must be solved in order to proceed

Battle System - Optional villagers to fight. Two bosses that must be fought

Dialogue - Interactable characters with some dialogue

World Map - Must go through the princess's castle to win the game

Color - Visible in actions and character dialogue. Player is also a different color.
